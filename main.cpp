	...
	for (int row = 0; row < maze_height; row++) {
		for (int col = 0; col < maze_width; col++) {
			...
			if (maze[row][col] == 't') {
				...
				cylinder->render(view * glm::translate(-15.0f,0.0f,-15.0f)*glm::translate((float)col * 10, 0.0f, (float)row * 10) * glm::scale(5.0f, 5.0f, 5.0f), projection);
			...
			if (maze[row][col] == 'x') {
				glPushMatrix();
				glTranslatef(maze_width, 0, maze_height);
				playerModel->render(view * glm::translate(-18.0f, 0.0f, -15.0f) * glm::translate((float)col*10, -5.0f, (float)row*10), projection);
				glPopMatrix();
			}
			if (maze[row][col] == 'g') {
				glPushMatrix();
				glTranslatef(maze_width, 0, maze_height);
				guns->render(view  *glm::translate(-18.0f, 2.0f, -15.0f) * glm::translate((float)col * 10, -5.0f, (float)row * 10) * glm::rotate(delta, 0.0f, 1.0f, 0.0f), projection);
				glPopMatrix();
			}
